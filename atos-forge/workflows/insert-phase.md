...
```bash
INIT=$(node ~/.claude/atos-forge/bin/forge-tools.cjs init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node ~/.claude/atos-forge/bin/forge-tools.cjs phase insert "${after_phase}" "${description}")
```
