...
```bash
INIT=$(node ~/.claude/atos-forge/bin/forge-tools.cjs init quick "$DESCRIPTION")
```
...
```bash
node ~/.claude/atos-forge/bin/forge-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
