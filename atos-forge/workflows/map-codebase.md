...
```bash
INIT=$(node ~/.claude/atos-forge/bin/forge-tools.cjs init map-codebase)
```
...
```bash
node ~/.claude/atos-forge/bin/forge-tools.cjs commit "docs: map existing codebase" --files .planning/codebase/*.md
```
